...
/**
 * Debug commands for crafting.
 *
 * These live here rather than in the commands module so that module does not have to depend on
 * crafting. `requiredRights` is set explicitly because the commands module's own `onCommand` helper
 * applies it for every admin command, and that helper is internal to it.
 */
