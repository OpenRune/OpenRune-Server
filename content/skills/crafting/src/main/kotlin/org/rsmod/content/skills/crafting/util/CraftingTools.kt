...
/** The equivalency mapping for tools */
...
/** Tools that also satisfy a requirement from a worn slot (any hand). */
private val WORN_TOOLS: Set<String> = setOf(
    CraftingConstants.IMCANDO_HAMMER,
    CraftingConstants.IMCANDO_HAMMER_OFFHAND,
)

/** The objs that satisfy [tool]. This includes the tool itself. */
fun toolEquivalents(tool: String): List<String> = TOOL_EQUIVALENTS[tool] ?: listOf(tool)

/** Whether the player has [tool] or any of its equivalents, held or worn. */
fun ProtectedAccess.hasCraftingTool(tool: String): Boolean = toolEquivalents(tool).any { holds(it) }

/** True if the player is holding an imcando hammer. */
...
/** Whether [tool] is in the inventory, or worn when it is one that counts from a hand slot. */
