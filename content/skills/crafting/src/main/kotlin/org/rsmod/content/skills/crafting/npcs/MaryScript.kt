...
class MaryScript : PluginScript() {
    ...
    override fun ScriptContext.startup() {
        if (!CraftingGamevals.exists(TANNER_MARY)) {
            return
        }
    ...
    /** The pre-quest Mary, who has standard dialogue with no options and no tanning. */
    ...
    /** The one-off conversation straight after Getting Ahead. */
    private suspend fun Dialogue.gettingAheadAftermath() {
        ...
        chatNpc(happy, "Not to worry. If it keeps Gordon happy, so be it. Anyway, thank you for keeping our " +
                        "farm safe. With the beast dealt with, I've been able to get back to tanning again.",
        )

        val tan = choice2( "Could you tan something for me?", true,
    ...
    private fun ProtectedAccess.tansForPlayer(): Boolean = player.hasCompletedQuest(QUEST_GETTING_AHEAD)
}
...
/** Set once Mary has given her post-Getting Ahead speech. */
