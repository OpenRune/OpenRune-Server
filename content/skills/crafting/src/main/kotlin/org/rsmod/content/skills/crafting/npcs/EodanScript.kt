...
class EodanScript : PluginScript() {
    ...
    override fun ScriptContext.startup() {
        if (!CraftingGamevals.exists(TANNER_EODAN)) {
            return
        }
...
/** Eodan's price schedule per Kourend & Kebos diary tier. */
...
private fun ProtectedAccess.eodanTier(): EodanTier {
    ...
    for ((index, varbit) in KOUREND_DIARY_VARBITS.withIndex()) {
...
/** Lunar spellbook and 78 Magic, the requirements for Tan Leather, which he grumbles about. */
private fun ProtectedAccess.canCastTanLeather(): Boolean {
    if (!CraftingGamevals.exists(VARBIT_SPELLBOOK)) {
        return false
    }
...
private val KOUREND_DIARY_VARBITS: List<String> = listOf(
...
/** Active spellbook varbit, where 2 is Lunar. */
