...
class DiscordLinkScript @Inject constructor(private val centralDiscord: CentralDiscordWorldLink) :
    PluginScript() {
    override fun ScriptContext.startup() {
        onEvent<SessionStateEvent.Logout> { centralDiscord.invalidatePending(player.accountId) }
    ...
    private fun Cheat.handleDiscordLink() {
        ...
        if (discordUsername.isBlank()) {
            player.mes(
                "Could not determine your Discord username. Use: ::discordlink <discord_username>"
            )
            return
        }
        ...
        when (
            ...
            is CentralDiscordWorldLink.GamePendingResult.Ok -> {
                ...
                if (result.dmSent) {
                    ...
                    player.mes(
                        "Could not send a Discord DM. Check your privacy settings and try again."
                    )
                }
            }
            CentralDiscordWorldLink.GamePendingResult.DiscordNotFound -> {
                player.mes(
                    "That Discord user was not found. They must be a member of the Discord server."
                )
            }
