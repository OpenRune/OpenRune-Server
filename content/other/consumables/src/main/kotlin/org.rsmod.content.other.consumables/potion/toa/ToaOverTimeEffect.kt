...
class ToaOverTimeEffect {
    fun applySilkDressing(access: ProtectedAccess) {
        with(access) {
            restoreIfDrained(stat = HITPOINTS, constant = SILK_DRESSING_HEAL, percent = 0)
    ...
    fun processSilkDressing(access: ProtectedAccess) {
        with(access) {
            processPulse(
                state = SILK_DRESSING_PULSES,
                timer = SILK_DRESSING_TIMER,
                interval = SILK_DRESSING_INTERVAL,
                expiryMessage = "The effects of the silk dressing have worn off.",
            ) {
                restoreIfDrained(stat = HITPOINTS, constant = SILK_DRESSING_HEAL, percent = 0)
            }
        }
    ...
    fun applyBlessedCrystalScarab(access: ProtectedAccess) {
        with(access) {
            restoreIfDrained(stat = PRAYER, constant = BLESSED_SCARAB_RESTORE, percent = 0)
    ...
    fun processBlessedCrystalScarab(access: ProtectedAccess) {
        with(access) {
            processPulse(
                state = BLESSED_SCARAB_PULSES,
                timer = BLESSED_SCARAB_TIMER,
                interval = BLESSED_SCARAB_INTERVAL,
                expiryMessage = "The effects of the blessed crystal scarab have worn off.",
            ) {
                restoreIfDrained(stat = PRAYER, constant = BLESSED_SCARAB_RESTORE, percent = 0)
            }
        }
    ...
    fun clear(player: Player) {
        clearPulses(player = player, state = SILK_DRESSING_PULSES, timer = SILK_DRESSING_TIMER)

        clearPulses(player = player, state = BLESSED_SCARAB_PULSES, timer = BLESSED_SCARAB_TIMER)
    }
    ...
    private inline fun ProtectedAccess.processPulse(
        ...
        val remaining = player.attr[state] ?: return

        apply()

        if (remaining <= 1) {
            clearPulses(player = player, state = state, timer = timer)

            mes(expiryMessage)
            return
        }

        player.attr[state] = remaining - 1

        player.timer(timer, interval)
    }

    private fun clearPulses(player: Player, state: AttributeKey<Int>, timer: String) {
        player.attr.remove(state)
        player.clearTimer(timer)
    }

    companion object {
        const val SILK_DRESSING_TIMER: String = "timer.potion_toa_silk_dressing"

        const val BLESSED_SCARAB_TIMER: String = "timer.potion_toa_blessed_crystal_scarab"
