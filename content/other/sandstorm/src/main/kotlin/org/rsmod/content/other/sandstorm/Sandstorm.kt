...
class Sandstorm : PluginScript() {
    ...
    private suspend fun ProtectedAccess.useObjOnGrinder(objType: ItemServerType) {
        if (objType.internalName !in sandstoneObjs) {
    ...
    private suspend fun ProtectedAccess.depositAtGrinder() {
        ...
        if (deposit.deposited <= 0) {
            if (deposit.overflowed) {
                mes("The grinder is full of sandstone.")
            } else {
                mes("You do not have any sandstone to grind.")
            }
            return
        }
...
private val SANDSTORM_BUCKETS_EMPTY = AttributeKey<Int>(persistenceKey = "sandstorm_buckets_empty")
private val SANDSTORM_BUCKETS_SAND = AttributeKey<Int>(persistenceKey = "sandstorm_buckets_sand")

internal enum class Sandstone(val obj: String, val sand: Int) {
    Large("obj.enakh_sandstone_large", 8),
    Medium("obj.enakh_sandstone_medium", 4),
    Small("obj.enakh_sandstone_small", 2),
    Tiny("obj.enakh_sandstone_tiny", 1),
}

internal val sandstoneObjs: Set<String> = Sandstone.entries.map { it.obj }.toSet()

internal var Player.sandstormBucketsEmpty: Int
    get() = attr.getOrDefault(SANDSTORM_BUCKETS_EMPTY, 0)
    set(value) {
        attr[SANDSTORM_BUCKETS_EMPTY] = value.coerceIn(0, MAX_STORED)
    }

internal var Player.sandstormBucketsSand: Int
    get() = attr.getOrDefault(SANDSTORM_BUCKETS_SAND, 0)
    set(value) {
        attr[SANDSTORM_BUCKETS_SAND] = value.coerceIn(0, MAX_STORED)
    }
...
internal fun ProtectedAccess.depositSandstone(): SandstoneDeposit {
    var space = MAX_STORED - player.sandstormBucketsSand
    var deposited = 0
    var overflowed = false
    for (sandstone in Sandstone.entries) {
        val held = inv.count(sandstone.obj)
        if (held <= 0) {
            continue
        }
        val deposit = minOf(held, space / sandstone.sand)
        if (deposit < held) {
            overflowed = true
        }
        if (deposit <= 0) {
        ...
        if (invDel(inv, sandstone.obj, deposit).failure) {
            continue
        }
        val sand = deposit * sandstone.sand
        player.sandstormBucketsSand += sand
        space -= sand
        deposited += deposit
    }
