...
/**
 * Columns that aren't specified defer to the section default. Every crafting table shares one
 * column layout, so a recipe uses the same columns regardless of the section or table it belongs to.
 *
...
object Crafting {
    ...
    const val COL_COST = 17

    /** An extra tool the recipe requires beyond any listed in it's section. */
    const val COL_TOOL = 18

    const val COL_SOUND = 19

    /** Success message override. {output} and {input} get interpolated in. */
    const val COL_SPAM_MESSAGE = 20

    const val COL_ACTION_NAME = 21

    const val COL_CONFIRM_TITLE = 22
    const val COL_CONFIRM_WARNING = 23
    const val COL_RESULT_DIALOGUE = 24

    const val COL_QUEST_REQ = 25
    const val COL_UNLOCK_VARBIT = 26
    const val COL_LOCKED_MESSAGE = 27
    ...
    fun hand() = craftingTable("dbtable.crafting_hand") {
        ...
        section("Limestone", category = "Cut") {
            // 137/434 out of 256 reproduces the wiki curve. ~33% failure at level 12 and 100% to success at level 40.
        ...
        section("Combining") {
            ...
            row("dbrow.crafting_amulet_of_rancour") {
                ...
                column(COL_CONFIRM_TITLE, "Do you wish to create a amulet of rancour?") //misspelling on purpose since it's accurate
    ...
    fun silver() = craftingTable("dbtable.crafting_silver") {
    ...
    fun gold() = craftingTable("dbtable.crafting_gold") {
...
private class CraftingSectionScope(
     ...
     * `category(...)` (needlework's hide families) still takes priority.
     * Later writes replace earlier ones for the same column.
     */
...
/** How a recipe's `quest_req` column reads a quest's state. Mirrors the quest manager's own enum. */
...
/** How a recipe's `unlock_varbit` column compares the varbit against its value. */
