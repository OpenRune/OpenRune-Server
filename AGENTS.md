...
- Cache tasks (or-cache module): `buildCache`, `freshCache`, `cleanCs2`,
  `mergePluginGamevals`, `dumpLocs`, `dumpSeqs`. Cache data comes from OpenRS2 `.data/`.
- `gradlew :or-cache:dumpLocs -Plocs=loc.bob,12345` prints every tile a loc is placed on - plane,
  angle, shape, cache name and ops. `-Plocs=x1:z1:x2:z2` dumps a box instead. This is how you find
  where scenery actually stands without logging in; obstacle landings and map edits both need it.
- `gradlew :or-cache:dumpSeqs -Pseqs=seq.name,1234` prints how many ticks an animation runs for.
  Anything that carries a player has to last as long as its animation, or they arrive ahead of their
  own feet.
...
deviations), `agility-verification.md` (in-client checklist for the courses),
