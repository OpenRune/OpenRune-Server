...
fun adamantDragon(npcType: String): BossSpec = boss(npcType) {
    ...
    val meleeFire = ability("melee_dragonfire") {
        anim(DRAGON_BREATH_ANIM)
        include(Effect.Spotanim("spotanim.firebreath_attack", height = 100))
        hit { damage(0..50).roll(); type(DragonfireMetal) }
    }
    val rangedFire = ability("ranged_dragonfire") {
        anim(DRAGON_BREATH_ANIM)
        projectile(
...
fun mithrilDragon(npcType: String): BossSpec = boss(npcType) {
    ...
    val magicalRanged = ability("magical_ranged") {
        anim("seq.brut_dragon_casting")
        projectile(
