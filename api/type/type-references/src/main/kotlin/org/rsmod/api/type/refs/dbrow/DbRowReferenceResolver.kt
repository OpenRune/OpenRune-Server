...
public class DbRowReferenceResolver
@Inject
constructor(private val nameMapping: NameMapping, private val types: DbRowTypeList) :
    TypeReferenceResolver<HashedDbRowType> {
    private val logger = InlineLogger()

    private val names: Map<String, Int>
        get() = nameMapping.dbRows

    override fun resolve(refs: TypeReferences<HashedDbRowType>): List<TypeReferenceResult> =
        refs.cache.map { it.resolve() }

    private fun HashedDbRowType.resolve(): TypeReferenceResult {
        ...
        if (autoResolve) {
            ...
            logger.trace { "  DbRow($name) identity hash auto-resolved: $cacheIdentityHash" }
